...
export default {
  ...
  methods: {
    cargarReceta() {
      ...
      if (recetaAEditar) {
        // Usamos spread (...) para copiar el objeto y no modificar el original
    ...
    actualizarReceta() {
      ...
      // Buscamos el *index* de la receta en el array original
      const index = this.recetas.findIndex((r) => r.id === this.receta.id)

      if (index !== -1) {
        // Reemplazamos el objeto antiguo por el nuevo en esa posición
